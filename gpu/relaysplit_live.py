"""
RelaySplit LIVE — WebRTC audio -> streaming SCNet vocal separation -> WebRTC audio, on a warm,
region-pinned UK GPU. The moment of truth: play music into the browser, hear just the vocal come
back through a cloud GPU, with the latency measured on screen. (Model: SCNet-small, chosen over
htdemucs after an A/B; htdemucs remains the benchmark baseline in relaysplit_gpu.py.)

Design:
  - The container is a WebRTC peer (aiortc). Inbound 48 kHz audio -> stateful resample to 44.1 kHz
    -> OnlineSeparator (adaptive hop / 5 s context / short crossfade) -> resample back to 48 kHz ->
    outbound track. Inference runs OFF the event loop so audio never stalls. Signalling works both
    self-contained (POST /offer) and via the VPS /ws control plane.
...
VPS_BASE = "https://relaysplit.vaguelystrange.com"
TURN_ENDPOINT = f"{VPS_BASE}/api/turn"
# Self URL for the keep-warm ping. Cost discipline: nothing external polls this container (clients poll
# the VPS instead), so it scales to zero when idle. While sessions ARE active the container pings itself
# to stay warm; when the last peer disconnects the pinging stops and Modal scales it down.
SELF_URL = "https://blitzncs--relaysplit-live-web.modal.run"
...
# alone can hit capacity waits, which would leave the user staring at "waiting for GPU".
...
# GPU preference ordered by MEASURED fp16 forward time (L40S 48ms < L4 84ms < A100 93 < A10G 114 —
...
def web():
    ...
    async def demo():
        # Same page, flagged to auto-use the baked track as source — one-click live demo for a
        # visitor (no mic/file needed). Autoplay policy still requires the single Start click.
    ...
    @api.get("/ping")
    async def ping():
        return JSONResponse({"ok": True})  # target of the self keep-warm ping
    ...
    async def report_broadcasts_loop():
        # Tell the VPS which channels are live, so clients poll the VPS (cheap) instead of this GPU.
        # Outbound only, so it never resets our scale-to-zero timer. When the container scales down this
        # stops and the VPS entries expire on their own (short TTL).
        loop = asyncio.get_event_loop()
        while True:
            try:
                ids = [int(k) for k, b in broadcasts.items() if b.source is not None and str(k).isdigit()]
                if ids:
                    def _post():
                        req = urllib.request.Request(
                            VPS_BASE + "/api/broadcasts",
                            data=json.dumps({"channels": ids}).encode(),
                            headers={"Content-Type": "application/json"}, method="POST")
                        urllib.request.urlopen(req, timeout=5).read()
                    await loop.run_in_executor(None, _post)
            except Exception as e:
                log.warning("broadcast report failed: %s", e)
            await asyncio.sleep(5)

    async def self_keepalive_loop():
        # Stay warm ONLY while peers are connected. A self-request counts as inbound activity to Modal
        # (resetting scaledown); we send it every 120 s (well under the 300 s window) but ONLY when `pcs`
        # is non-empty. When the last peer leaves, the pings stop and the container scales to zero.
        loop = asyncio.get_event_loop()
        while True:
            ...
            if not pcs:
                continue
            try:
                await loop.run_in_executor(None, lambda: urllib.request.urlopen(SELF_URL + "/ping", timeout=5).read())
            except Exception as e:
                log.warning("self keepalive failed: %s", e)

    @api.on_event("startup")
    async def _on_startup():
        asyncio.ensure_future(ws_session_loop())
        asyncio.ensure_future(report_broadcasts_loop())
        asyncio.ensure_future(self_keepalive_loop())
