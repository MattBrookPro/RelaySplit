"""
RelaySplit GPU — the OFFLINE BENCHMARK + DEV HARNESS (not the live path). It runs Meta's
**Demucs v4 / htdemucs**, which is the quality BASELINE the production model was measured against,
and provides the latency sweep, the per-GPU forward benchmark (`knee`, `gpubench`), and the offline
streaming validation.

The LIVE container is `relaysplit_live.py`, which runs **SCNet-small** (chosen over htdemucs after an
A/B in `relaysplit_scnet_ab.py`: ~2.5x faster forward and cleaner vocals). The brief's causal
Conv-TasNet was tried first and abandoned for quality. HS-TasNet is the causal upgrade path (no public
weights). Both htdemucs and SCNet are non-causal and full-band 44.1 kHz, so the latency model below
applies to either.

LATENCY MODEL (the whole game). The model is non-causal, so to emit audio for [t, t+chunk] you must
...
quality allows. `latency_sweep` measures this trade-off empirically so we pick the knee.
    ...
    modal run gpu/relaysplit_gpu.py::knee                    # fp16 real-time knee (hop vs context)
